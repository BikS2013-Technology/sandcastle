...
| Apple Containers  | `@ai-hero/sandcastle/sandboxes/apple-containers`  | Bind-mount | `run()`, `createSandbox()`, `interactive()` (macOS-arm64 only)                                        |
| Vercel            | `@ai-hero/sandcastle/sandboxes/vercel`            | Isolated   | `run()`, `createSandbox()`, `interactive()`                                                           |
| GitHub Codespaces | `@ai-hero/sandcastle/sandboxes/github-codespaces` | Isolated   | `run()`, `createSandbox()` (merge-to-head and branch strategies only; `existing` and `managed` modes) |
